...
def main(load_profile_input, solar_profile_input, customer_class_input, solar_system_kw_input, storage_power_kw_input, storage_capacity_kwh_input):

    # OSESMO Git Repository Directory Location and Import/Output Data Directory Location
    OSESMO_Git_Repo_Directory = Input_Output_Data_Directory_Location = os.path.abspath(os.path.dirname(__file__))
    ...
    Modeling_Team_Input = "StoreHouse"
    ...
    Customer_Class_Input = customer_class_dict[customer_class_input]

    # Load Profile Name
    # Name of the selected customer load profile.
    Load_Profile_Name_Input = "Customer load profile"

    # EDIT FROM JACKSON HERRON for STOREHOUSE
    # load_profile_input is the 8760 data of the load profile in kwh
    Load_Profile_Input = load_profile_input

    # Retail Rate Name
    # Name of the selected retail rate.

    Retail_Rate_Name_Input = "PG&E E-TOU-B"
    # Retail_Rate_Name_Input = ' '.join([utility_name_input, tariff_name_input])
    ...
    Solar_Profile_Name_Input = "Customer solar profile"

    # EDIT FROM JACKSON HERRON for STOREHOUSE
    # solar_profile_input is the 8760 data of the solar profile in kwh
    Solar_Profile_Input = solar_profile_input

    # Solar Size kW
    # Nameplate power rating of the solar PV system (kW-DC).
    Solar_Size_Input = solar_system_kw_input
    ...
    Storage_Power_Rating_Input = storage_power_kw_input
    ...
    Usable_Storage_Capacity_Input = storage_capacity_kwh_input

    # Single-Cycle RTE
    # Single-cycle (or nameplate) storage round-trip efficiency (50#, 70#, or 85#).
    Single_Cycle_RTE_Input = 0.96
    ...
    from services.OSESMO.Storage_Installed_Cost_per_kWh_Calculator import Storage_Installed_Cost_per_kWh_Calculator
    ...
    from services.OSESMO.OSESMO import OSESMO

    return OSESMO(Modeling_Team_Input, Model_Run_Number_Input, Model_Type_Input,
        Model_Timestep_Resolution, Customer_Class_Input, Load_Profile_Name_Input,
        Load_Profile_Input, Retail_Rate_Name_Input,
        Solar_Profile_Name_Input, Solar_Profile_Input, Solar_Size_Input,
